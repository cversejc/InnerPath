  ...
  <div class="admin-shell">
    ...
    <main class="admin-main">
        ...
        <div class="dashboard-toolbar">
          ...
          <div class="toolbar-controls">
            <div class="range-switch" role="group" aria-label="数据范围">
              <button v-for="range in dashboardRanges" :key="range.id" type="button" :class="{ active: dashboardRange === range.id }" :aria-pressed="dashboardRange === range.id" @click="changeDashboardRange(range.id)">{{ range.label }}</button>
            </div>
        ...
        <template v-else-if="dashboard">
          ...
          <div class="dashboard-grid">
            ...
            <article class="dashboard-panel activity-panel">
              <div class="panel-heading"><div><p class="eyebrow">TRACE / LATEST 8</p><h3>最近活动</h3></div><button type="button" class="panel-link" @click="switchTab('logs')"><span>查看日志</span><IconMark name="arrow" /></button></div>
      ...
      <section v-else-if="activeTab === 'users'" class="content-view">
        <div class="view-heading"><div><p class="eyebrow">PEOPLE / DIRECTORY</p><h2>用户运营</h2><p>从账户状态到成长轨迹，统一查看和维护。</p></div><button class="secondary-button" type="button" @click="exportResource('users')"><IconMark name="download" /> <span>导出用户 CSV</span></button></div>
        <div class="filter-bar">
          <input v-model.trim="userFilters.search" aria-label="搜索姓名或手机号" placeholder="搜索姓名或手机号" @keyup.enter="searchUsers">
          <select v-model="userFilters.role" aria-label="按角色筛选"><option value="">全部角色</option><option value="user">用户</option><option value="consultant">咨询师</option><option value="admin">管理员</option></select>
          <select v-model="userFilters.is_active" aria-label="按状态筛选"><option value="">全部状态</option><option :value="true">正常</option><option :value="false">已停用</option></select>
      ...
      <section v-else-if="activeTab === 'bookings'" class="content-view">
        <div class="view-heading"><div><p class="eyebrow">SERVICE / APPOINTMENTS</p><h2>预约运营</h2><p>从待确认到完成服务，掌握每一次交付节点。</p></div><button class="secondary-button" type="button" @click="exportResource('bookings')"><IconMark name="download" /> <span>导出预约 CSV</span></button></div>
        <div class="filter-bar">
          <input v-model.trim="bookingFilters.search" aria-label="搜索用户、手机号或服务" placeholder="搜索用户、手机号或服务" @keyup.enter="searchBookings">
          <select v-model="bookingFilters.status" aria-label="按预约状态筛选"><option value="">全部状态</option><option value="pending">待确认</option><option value="confirmed">已确认</option><option value="completed">已完成</option><option value="cancelled">已取消</option></select>
          <select v-model="bookingFilters.consultant_id" aria-label="按咨询师筛选"><option value="">全部咨询师</option><option v-for="consultant in consultants" :key="consultant.id" :value="consultant.id">{{ consultant.name }}</option></select>
         ...
         <div v-if="bookingsLoading" class="list-loading" aria-label="正在加载预约"><i v-for="index in 4" :key="index"></i></div><div v-else class="table-panel">
          ...
          <div class="admin-table-wrap" tabindex="0" aria-label="数据表格，可横向滚动查看"><table class="admin-table booking-table"><thead><tr><th>用户</th><th>服务</th><th>期望时间</th><th>状态</th><th>咨询师</th><th>创建时间</th><th>操作</th></tr></thead><tbody>
      ...
      <section v-else-if="activeTab === 'calendar'" class="content-view calendar-view">
        ...
        <div class="calendar-admin-grid">
          <aside class="user-directory panel-surface"><div class="panel-heading"><div><p class="eyebrow">SELECT USER</p><h3>选择用户</h3></div><span>{{ calendarUsers.length }}</span></div><div class="directory-search"><input v-model.trim="calendarUserSearch" aria-label="搜索用户" placeholder="搜索用户" @keyup.enter="loadCalendarUsers"><button type="button" aria-label="搜索用户" @click="loadCalendarUsers"><IconMark name="search" /></button></div><div class="directory-list"><button v-for="user in calendarUsers" :key="user.id" type="button" :class="{ selected: selectedCalendarUser?.id === user.id }" :aria-pressed="selectedCalendarUser?.id === user.id" @click="selectCalendarUser(user)"><span class="avatar-mark small">{{ user.name?.slice(0, 1) || '人' }}</span><span><strong>{{ user.name }}</strong><small>#{{ user.id }} · {{ user.phone }}</small></span><IconMark name="arrow" /></button><p v-if="!calendarUsers.length" class="empty-cell">请搜索或暂无用户。</p></div></aside>
          <div class="calendar-editor panel-surface"><div class="panel-heading"><div><p class="eyebrow">CALENDAR VERSIONS</p><h3>{{ selectedCalendarUser ? `${selectedCalendarUser.name} 的日历` : '先选择一个用户' }}</h3></div><button v-if="selectedCalendarUser" class="primary-button compact-button" type="button" @click="startNewCalendar"><IconMark name="plus" /> <span>新建草稿</span></button></div>
            ...
            <div v-else-if="selectedCalendarUser" class="editor-empty"><IconMark name="calendar" /><p>选择一个版本开始编辑，或创建一张新的草稿日历。</p></div><div v-else class="editor-empty"><IconMark name="compass" /><p>从左侧选择用户后，这里会显示其全部日历版本。</p></div>
          </div>
        </div>
      </section>

      <section v-else-if="activeTab === 'reports'" class="content-view">
        <div class="view-heading"><div><p class="eyebrow">AI / REPORT PIPELINE</p><h2>报告与任务</h2><p>查看全局生成状态，失败任务可受控重试。</p></div><button class="secondary-button" type="button" @click="exportResource('reports')"><IconMark name="download" /> <span>导出报告 CSV</span></button></div>
      ...
      <section v-else-if="activeTab === 'staff'" class="content-view">
        ...
        <div class="staff-grid"><article class="panel-surface invite-card"><p class="eyebrow">NEW INVITATION</p><h3>邀请后台成员</h3><form class="stack-form" @submit.prevent="inviteStaff"><label>手机号<input v-model.trim="inviteForm.phone" type="tel" inputmode="numeric" autocomplete="tel" required maxlength="11" placeholder="11 位手机号"></label><label>角色<select v-model="inviteForm.role"><option value="consultant">咨询师</option><option value="admin">管理员</option></select></label><button class="primary-button" type="submit" :disabled="inviteSaving" :aria-busy="inviteSaving">{{ inviteSaving ? '生成中…' : '生成邀请链接' }}</button></form><div v-if="inviteToken" class="invite-result" role="status" aria-live="polite"><span>本次令牌</span><code>{{ inviteToken }}</code><router-link :to="{ path: '/auth/invite', query: { token: inviteToken } }">打开邀请页面 →</router-link></div></article><article class="panel-surface team-card"><div class="panel-heading"><div><p class="eyebrow">CURRENT TEAM</p><h3>当前成员</h3></div><span>{{ staffUsers.length }}</span></div><div class="team-list"><div v-for="member in staffUsers" :key="member.id" class="team-row"><span class="avatar-mark">{{ member.name?.slice(0, 1) || '人' }}</span><div><strong>{{ member.name }}</strong><small>{{ roleText(member.role) }} · {{ member.phone }}</small></div><span :class="['status-badge', member.is_active ? 'success' : 'muted']">{{ member.is_active ? '正常' : '停用' }}</span></div><p v-if="!staffUsers.length" class="empty-cell">暂无后台成员。</p></div></article></div>
      </section>
    </main>

    <div v-if="detailUser" class="drawer-layer" @click.self="closeUserDetail">
      <aside ref="userDrawer" class="drawer user-drawer" role="dialog" aria-modal="true" aria-labelledby="user-detail-title" tabindex="-1" @keydown="handleDrawerKeydown"><div class="drawer-header"><div class="person-cell"><span class="avatar-mark large">{{ detailUser.name?.slice(0, 1) || '人' }}</span><span><p class="eyebrow">USER #{{ detailUser.id }}</p><h2 id="user-detail-title">{{ detailUser.name }}</h2><small>{{ detailUser.phone }}</small></span></div><button type="button" class="drawer-close" aria-label="关闭用户详情" @click="closeUserDetail">×</button></div><div class="drawer-tabs"><button v-for="tab in userPanelTabs" :key="tab.id" type="button" :class="{ active: userPanelTab === tab.id }" @click="setUserPanelTab(tab.id)">{{ tab.label }}</button></div><div v-if="userPanelLoading" class="drawer-loading" role="status" aria-live="polite">正在整理用户资料…</div><div v-else class="drawer-body">
    ...
    <div v-if="bookingDetail" class="drawer-layer" @click.self="closeBookingDetail"><aside ref="bookingDrawer" class="drawer booking-drawer" role="dialog" aria-modal="true" aria-labelledby="booking-detail-title" tabindex="-1" @keydown="handleDrawerKeydown"><div class="drawer-header"><div><p class="eyebrow">BOOKING #{{ bookingDetail.id }}</p><h2 id="booking-detail-title">{{ bookingDetail.service_name }}</h2><small>{{ bookingDetail.user_name || `用户 #${bookingDetail.user_id}` }} · {{ bookingDetail.user_phone || bookingDetail.contact_phone }}</small></div><button type="button" class="drawer-close" aria-label="关闭预约详情" @click="closeBookingDetail">×</button></div><form class="drawer-body stack-form" @submit.prevent="saveBooking"><div class="detail-facts"><p><span>关注议题</span><strong>{{ bookingDetail.topics?.join('、') || '—' }}</strong></p><p><span>用户备注</span><strong>{{ bookingDetail.notes || '—' }}</strong></p><p><span>期望时间</span><strong>{{ bookingDetail.preferred_time }}</strong></p></div><label>预约状态<select v-model="bookingEditor.status"><option value="pending">待确认</option><option value="confirmed">已确认</option><option value="completed">已完成</option><option value="cancelled">已取消</option></select></label><div class="form-grid two"><label>确认日期<input v-model="bookingEditor.confirmed_date" type="date"></label><label>确认时间<input v-model="bookingEditor.confirmed_time" type="time"></label></div><label>分配咨询师<select v-model="bookingEditor.consultant_id"><option :value="null">未分配</option><option v-for="consultant in consultants" :key="consultant.id" :value="consultant.id">{{ consultant.name }}</option></select></label><label>会议链接<input v-model.trim="bookingEditor.meeting_url" type="url" placeholder="可选"></label><label>会议备注<textarea v-model.trim="bookingEditor.meeting_notes" rows="4" placeholder="记录会议安排或沟通要点"></textarea></label><label v-if="bookingEditor.status === 'cancelled'">取消原因<textarea v-model.trim="bookingEditor.cancellation_reason" rows="3" placeholder="可选"></textarea></label><button class="primary-button" type="submit" :disabled="bookingSaving" :aria-busy="bookingSaving">{{ bookingSaving ? '保存中…' : '保存预约' }}</button></form></aside></div>

    <div v-if="reportDetail" class="drawer-layer" @click.self="closeReportDetail"><aside ref="reportDrawer" class="drawer report-drawer" role="dialog" aria-modal="true" aria-labelledby="report-detail-title" tabindex="-1" @keydown="handleDrawerKeydown"><div class="drawer-header"><div><p class="eyebrow">REPORT #{{ reportDetail.id }}</p><h2 id="report-detail-title">{{ reportDetail.title }}</h2><small>{{ reportDetail.user_name }} · {{ reportDetail.user_phone }}</small></div><button type="button" class="drawer-close" aria-label="关闭报告详情" @click="closeReportDetail">×</button></div><div class="drawer-body report-body"><div class="report-meta-grid"><div><span>状态</span><strong>{{ reportStatusText(reportDetail.status) }}</strong></div><div><span>模型</span><strong>{{ reportDetail.ai_model || '—' }}</strong></div><div><span>耗时</span><strong>{{ reportDetail.generation_time_ms ? `${reportDetail.generation_time_ms} ms` : '—' }}</strong></div><div><span>生成于</span><strong>{{ formatDateTime(reportDetail.created_at) }}</strong></div><div><span>生成来源</span><strong>{{ reportDetail.basic_info?.generated_by || '—' }}</strong></div></div><article class="report-block"><h3>摘要</h3><p>{{ reportDetail.summary || '暂无摘要。' }}</p></article><article class="report-block"><h3>能量画像</h3><pre>{{ prettyJson(reportDetail.energy_profile) }}</pre></article><article class="report-block"><h3>行动建议</h3><pre>{{ prettyJson(reportDetail.career_guidance) }}</pre></article><article class="report-block"><h3>关系模式</h3><pre>{{ prettyJson(reportDetail.relationship_pattern) }}</pre></article><article class="report-block"><h3>个人成长</h3><pre>{{ prettyJson(reportDetail.personal_growth) }}</pre></article></div></aside></div>

    <div v-if="logDetail" class="drawer-layer" @click.self="closeLogDetail"><aside ref="logDrawer" class="drawer log-drawer" role="dialog" aria-modal="true" aria-labelledby="log-detail-title" tabindex="-1" @keydown="handleDrawerKeydown"><div class="drawer-header"><div><p class="eyebrow">AUDIT #{{ logDetail.id }}</p><h2 id="log-detail-title">{{ actionLabel(logDetail.action) }}</h2><small>{{ formatDateTime(logDetail.created_at) }}</small></div><button type="button" class="drawer-close" aria-label="关闭审计详情" @click="closeLogDetail">×</button></div><div class="drawer-body"><div class="detail-facts"><p><span>操作编码</span><strong class="mono-text">{{ logDetail.action }}</strong></p><p><span>资源</span><strong>{{ resourceLabel(logDetail.resource_type) }} {{ logDetail.resource_id ? `#${logDetail.resource_id}` : '' }}</strong></p><p><span>操作者</span><strong>{{ logDetail.actor_name || logDetail.actor_user_id || '系统' }}</strong></p><p><span>目标用户</span><strong>{{ logDetail.target_user_name || logDetail.target_user_id || '—' }}</strong></p><p><span>请求 ID</span><strong class="mono-text">{{ logDetail.request_id || '—' }}</strong></p><p><span>IP / UA</span><strong>{{ logDetail.ip_address || '—' }}<small>{{ logDetail.user_agent || '' }}</small></strong></p></div><div class="json-view"><span>结构化详情</span><pre>{{ prettyJson(logDetail.details_json || logDetail.details || {}) }}</pre></div></div></aside></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDrawer(name) { const drawer = this.$refs[name]; return Array.isArray(drawer) ? drawer[0] : drawer },
    ...
    handleDrawerKeydown(event) {
      if (event.key === 'Escape') { event.preventDefault(); this.closeActiveDrawer(); return }
      if (event.key !== 'Tab') return
      const drawer = event.currentTarget
      const focusables = [...drawer.querySelectorAll('button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [href], [tabindex]:not([tabindex="-1"])')].filter(element => element.offsetParent !== null)
...
<style scoped>
.admin-shell { min-height: 100dvh; background: radial-gradient(circle at 84% 4%, rgba(217, 186, 98, .16), transparent 22%), linear-gradient(135deg, var(--paper, #f8f1e6) 0%, #fffaf0 48%, #ead9bf 100%); }
...
.hero-caption, .view-heading p, .dashboard-toolbar p { color: var(--muted, #7d6653); line-height: 1.7; }
.hero-actions { display: flex; align-items: center; gap: 14px; padding-bottom: 3px; }
.sync-state { display: inline-flex; align-items: center; gap: 8px; color: var(--muted, #7d6653); font-family: Manrope, "PingFang SC", sans-serif; font-size: 11px; letter-spacing: .04em; white-space: nowrap; }
...
.admin-account-button { display: inline-flex; min-height: 44px; align-items: center; justify-content: center; gap: 7px; border: 1px solid rgba(158, 63, 53, .3); border-radius: 10px; padding: 0 14px; background: rgba(255, 250, 240, .56); color: var(--cinnabar-deep, #9e3f35); cursor: pointer; font: inherit; font-size: 13px; font-weight: 800; transition: background var(--motion-standard, 220ms) var(--ease-out, ease), border-color var(--motion-standard, 220ms) var(--ease-out, ease), color var(--motion-standard, 220ms) var(--ease-out, ease), transform var(--motion-fast, 150ms) var(--ease-out, ease); }
.admin-account-button .icon-mark { width: 16px; height: 16px; }
.admin-account-button:hover { border-color: var(--cinnabar-deep, #9e3f35); background: rgba(158, 63, 53, .08); }
.admin-account-button:active { transform: scale(.985); }
.admin-account-button:focus-visible { outline: 3px solid rgba(181, 87, 76, .3); outline-offset: 2px; }
.admin-account-button:disabled { cursor: wait; opacity: .62; }
.admin-tabs { display: flex; flex-wrap: wrap; gap: 8px; padding: 20px 0 28px; }
.admin-tab { display: inline-flex; align-items: center; gap: 9px; border: 1px solid rgba(80, 54, 32, .14); border-radius: 999px; padding: 10px 15px; color: var(--muted, #7d6653); background: rgba(255, 250, 240, .56); cursor: pointer; transition: .2s ease; }
...
.compact-button { min-height: 44px; border-radius: 10px; padding: 0 14px; font-size: 13px; }
...
.range-switch button, .section-switch button { border-radius: 999px; padding: 8px 13px; color: var(--muted, #7d6653); cursor: pointer; font-size: 12px; }
.range-switch button.active, .section-switch button.active { background: #3b2d24; color: #fffaf0; }
.auto-refresh { display: inline-flex; align-items: center; gap: 6px; color: var(--muted, #7d6653); font-size: 11px; white-space: nowrap; }
...
.metric-card.metric-cinnabar { color: #b85c50; }.metric-card.metric-gold { color: var(--gold-deep, #8b5a14); }.metric-card.metric-jade { color: #4f8875; }.metric-card.metric-ink { color: #59483d; }
.metric-top { display: flex; justify-content: space-between; color: var(--muted, #7d6653); font-size: 11px; }.metric-top b { color: currentColor; font-family: Manrope, sans-serif; font-size: 10px; letter-spacing: .08em; }
.metric-card > strong { display: block; margin: 17px 0 5px; color: #30241b; font-family: Manrope, sans-serif; font-size: 31px; letter-spacing: -.06em; }.metric-card small { color: var(--muted, #7d6653); font-size: 10px; line-height: 1.5; }
.dashboard-grid { display: grid; grid-template-columns: minmax(0, 1.7fr) minmax(300px, .8fr); gap: 18px; }.dashboard-panel { border-radius: 18px; padding: 22px; }.trend-panel, .distribution-panel { min-width: 0; }.trend-panel { grid-row: span 1; }.panel-heading { align-items: flex-start; }.panel-heading h3 { margin: 0; color: #3b2d24; font-size: 18px; }.panel-note, .alert-count, .panel-heading > span { color: var(--muted, #7d6653); font-family: Manrope, sans-serif; font-size: 10px; }.alert-count { display: inline-flex; width: 27px; height: 27px; align-items: center; justify-content: center; border-radius: 50%; background: #fff0df; color: #9e3f35; font-weight: 900; }
.trend-legend { display: flex; flex-wrap: wrap; gap: 14px; margin: 18px 0 4px; color: var(--muted, #7d6653); font-size: 11px; }.trend-legend span { display: inline-flex; align-items: center; gap: 6px; }.trend-legend i { width: 7px; height: 7px; border-radius: 50%; }.trend-chart { min-height: 250px; }.trend-chart svg { width: 100%; height: 250px; overflow: visible; }.chart-grid-line { stroke: rgba(80, 54, 32, .09); stroke-dasharray: 2 5; }.chart-tick { stroke: rgba(80, 54, 32, .2); }.chart-label { fill: var(--muted, #7d6653); font-family: Manrope, sans-serif; font-size: 10px; }.trend-line { fill: none; stroke-width: 2.8; stroke-linecap: round; stroke-linejoin: round; }
.alert-panel { min-height: 330px; }.alert-list { display: grid; gap: 8px; margin-top: 16px; }.alert-item { display: grid; grid-template-columns: 10px 1fr auto; align-items: center; gap: 10px; width: 100%; border: 1px solid rgba(80, 54, 32, .1); border-radius: 11px; padding: 12px; background: rgba(255, 250, 240, .6); text-align: left; cursor: pointer; }.alert-item:hover { border-color: rgba(184, 92, 80, .4); transform: translateX(2px); }.alert-item strong, .alert-item small { display: block; }.alert-item strong { color: #3b2d24; font-size: 13px; }.alert-item small { margin-top: 4px; color: var(--muted, #7d6653); font-size: 10px; }.alert-item > b { color: #b85c50; }.alert-mark { width: 8px; height: 8px; border-radius: 50%; }.alert-warning { background: var(--gold-deep, #8b5a14); box-shadow: 0 0 0 4px rgba(198, 155, 66, .14); }.alert-danger { background: #b85c50; box-shadow: 0 0 0 4px rgba(184, 92, 80, .13); }.alert-info { background: #6f9f93; box-shadow: 0 0 0 4px rgba(111, 159, 147, .13); }.quiet-state, .editor-empty { display: grid; place-items: center; gap: 8px; min-height: 210px; color: var(--muted, #7d6653); text-align: center; }.quiet-state span, .editor-empty span { color: var(--gold-deep, #8b5a14); font-size: 28px; }.quiet-state p, .editor-empty p { font-size: 12px; }
.distribution-panel { grid-column: 1 / -1; }.distribution-columns { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 20px; margin-top: 20px; }.distribution-group { min-width: 0; }.distribution-title { display: flex; justify-content: space-between; margin-bottom: 11px; color: #3b2d24; font-size: 12px; font-weight: 800; }.distribution-title small { color: var(--muted, #7d6653); font-family: Manrope, sans-serif; font-size: 10px; }.distribution-row { margin: 9px 0; }.distribution-row > div { display: flex; justify-content: space-between; margin-bottom: 5px; color: var(--muted, #7d6653); font-size: 11px; }.distribution-row b { color: #3b2d24; font-family: Manrope, sans-serif; }.distribution-track { display: block; height: 5px; border-radius: 999px; background: rgba(80, 54, 32, .1); overflow: hidden; }.distribution-track i { display: block; height: 100%; border-radius: inherit; background: linear-gradient(90deg, #b85c50, #d9ba62); }.activity-panel { min-height: 275px; }.panel-link { color: #9e3f35; font-size: 11px; font-weight: 800; cursor: pointer; }.activity-list { display: grid; gap: 0; margin-top: 12px; }.activity-item { display: grid; grid-template-columns: 10px 1fr auto; align-items: start; gap: 10px; padding: 11px 0; border-bottom: 1px solid rgba(80, 54, 32, .08); }.activity-item:last-child { border-bottom: 0; }.activity-dot { width: 7px; height: 7px; margin-top: 5px; border-radius: 50%; background: #b85c50; box-shadow: 0 0 0 4px rgba(184, 92, 80, .12); }.activity-item strong { color: #3b2d24; font-size: 12px; }.activity-item p, .activity-item time { margin-top: 3px; color: var(--muted, #7d6653); font-size: 10px; }.activity-item time { white-space: nowrap; }.activity-list.compact .activity-item { grid-template-columns: 10px 1fr auto; }
...
.import-panel { display: grid; grid-template-columns: minmax(180px, .7fr) minmax(0, 1.5fr) auto; align-items: center; gap: 14px; border: 1px dashed rgba(184, 92, 80, .35); border-radius: 14px; padding: 14px; background: rgba(255, 240, 223, .5); }.import-panel strong { color: #8f352f; }.import-panel p { margin-top: 4px; color: var(--muted, #7d6653); font-size: 11px; line-height: 1.5; }.import-panel textarea { width: 100%; resize: vertical; font-family: Consolas, monospace; font-size: 11px; }.calendar-admin-grid { display: grid; grid-template-columns: 290px minmax(0, 1fr); gap: 18px; align-items: start; }.panel-surface { border-radius: 17px; padding: 20px; }.user-directory { min-height: 620px; }.directory-search { display: flex; gap: 6px; margin: 17px 0 12px; }.directory-search input { width: 100%; }.directory-search button { width: 38px; border: 1px solid rgba(80, 54, 32, .14); border-radius: 8px; color: #9e3f35; cursor: pointer; }.directory-list { display: grid; gap: 7px; max-height: 690px; overflow-y: auto; }.directory-list button { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 9px; border: 1px solid rgba(80, 54, 32, .1); border-radius: 10px; padding: 9px; background: rgba(255, 250, 240, .66); text-align: left; cursor: pointer; }.directory-list button.selected { border-color: #b85c50; background: #fff0df; }.directory-list button strong { display: block; color: #3b2d24; font-size: 12px; }.directory-list button small { display: block; margin-top: 3px; color: var(--muted, #7d6653); font-size: 9px; }.directory-list button > b { color: #b85c50; }.calendar-editor { min-width: 0; min-height: 620px; }.calendar-list { display: grid; gap: 8px; margin-top: 17px; }.calendar-card { border: 1px solid rgba(80, 54, 32, .1); border-radius: 11px; padding: 12px; background: rgba(255, 250, 240, .52); }.calendar-card.selected { border-color: rgba(184, 92, 80, .56); background: rgba(255, 240, 223, .55); }.calendar-card-top { align-items: start; }.calendar-card-top strong { display: block; color: #3b2d24; font-size: 13px; }.calendar-card-top small { display: block; margin-top: 5px; color: var(--muted, #7d6653); font-size: 10px; }.calendar-card-preview { display: flex; flex-wrap: wrap; gap: 6px; margin: 11px 0; }.calendar-card-preview span { border-radius: 999px; padding: 4px 7px; background: rgba(234, 217, 191, .55); color: var(--muted, #7d6653); font-size: 9px; }.calendar-card .row-actions { justify-content: end; }.calendar-editor-form { margin-top: 18px; border-top: 1px solid rgba(80, 54, 32, .11); padding-top: 18px; }.editor-banner { margin-bottom: 14px; color: #8f352f; font-size: 12px; font-weight: 900; }.editor-banner span:last-child { color: var(--muted, #7d6653); font-size: 10px; font-weight: 400; }.form-grid { display: grid; gap: 10px; }.form-grid.two { grid-template-columns: repeat(2, minmax(0, 1fr)); }.form-grid.three { grid-template-columns: repeat(3, minmax(0, 1fr)); }.form-grid label, .stack-form label, .calendar-editor-form > label { display: grid; gap: 5px; color: var(--muted, #7d6653); font-size: 11px; font-weight: 700; }.form-grid input, .form-grid select { width: 100%; }.span-two { grid-column: span 2; }.entry-toolbar { margin: 22px 0 11px; }.entry-toolbar strong, .entry-toolbar small { display: block; }.entry-toolbar strong { color: #3b2d24; font-size: 14px; }.entry-toolbar small { margin-top: 3px; color: var(--muted, #7d6653); font-size: 10px; font-weight: 400; }.entry-list { display: grid; gap: 10px; }.entry-editor { display: grid; gap: 9px; border: 1px solid rgba(80, 54, 32, .12); border-radius: 12px; padding: 12px; background: rgba(255, 250, 240, .54); }.entry-editor-head { display: grid; grid-template-columns: 58px 1fr auto; align-items: end; gap: 9px; }.entry-editor-head > span { align-self: center; color: #b85c50; font-family: Manrope, sans-serif; font-size: 10px; font-weight: 900; }.entry-editor-head label { display: grid; gap: 4px; color: var(--muted, #7d6653); font-size: 10px; }.entry-editor-head button { align-self: center; color: #b85c50; font-size: 22px; cursor: pointer; }.calendar-editor-form textarea { resize: vertical; line-height: 1.5; }.preview-strip { flex-wrap: wrap; align-items: end; margin-top: 16px; border: 1px solid rgba(111, 159, 147, .22); border-radius: 10px; padding: 11px; background: rgba(225, 240, 220, .45); color: #5c786c; font-size: 10px; }.preview-strip strong { display: block; color: #39724e; font-size: 13px; }.preview-strip .eyebrow { margin-bottom: 4px; color: #5c786c; font-size: 8px; }.editor-actions { justify-content: end; margin-top: 16px; }.entry-empty { padding: 25px !important; }.section-switch { justify-self: start; }.section-switch button { min-width: 100px; }.staff-grid { display: grid; grid-template-columns: minmax(260px, .7fr) minmax(0, 1.3fr); gap: 18px; }.invite-card h3, .team-card h3 { margin: 0; color: #3b2d24; font-size: 20px; }.stack-form { display: grid; gap: 13px; margin-top: 18px; }.stack-form .primary-button { justify-self: start; }.invite-result { display: grid; gap: 8px; margin-top: 22px; border: 1px dashed rgba(184, 92, 80, .45); border-radius: 10px; padding: 14px; background: #fff4e9; }.invite-result span { color: var(--muted, #7d6653); font-size: 10px; }.invite-result code { color: #8f352f; overflow-wrap: anywhere; }.invite-result a { color: #8f352f; font-size: 12px; font-weight: 800; }.team-list { display: grid; gap: 8px; margin-top: 18px; }.team-row { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 10px; border-bottom: 1px solid rgba(80, 54, 32, .08); padding: 9px 0; }.team-row strong { color: #3b2d24; font-size: 13px; }.course-admin-list, .decision-list { display: grid; gap: 9px; }.course-admin-row, .decision-list > div { display: flex; align-items: center; justify-content: space-between; gap: 14px; border-bottom: 1px solid rgba(80, 54, 32, .09); padding: 11px 0; }.course-admin-row strong, .decision-list strong { color: #3b2d24; font-size: 12px; }.course-progress-editor { display: flex; align-items: center; gap: 3px; }.course-progress-editor input { width: 55px; padding: 7px; }.course-progress-editor span { color: var(--muted, #7d6653); font-size: 11px; }.course-progress-editor button { color: #9e3f35; font-size: 11px; font-weight: 800; cursor: pointer; }.full-button { width: 100%; margin-top: 17px; }.report-meta-grid { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 8px; }.report-meta-grid > div { border-radius: 9px; padding: 10px; background: rgba(234, 217, 191, .35); }.report-meta-grid span, .report-meta-grid strong { display: block; }.report-meta-grid span, .detail-facts span { color: var(--muted, #7d6653); font-size: 10px; }.report-meta-grid strong { margin-top: 5px; color: #3b2d24; font-size: 12px; }.report-block { margin-top: 20px; border-top: 1px solid rgba(80, 54, 32, .1); padding-top: 16px; }.report-block h3 { margin-bottom: 9px; color: #3b2d24; font-size: 14px; }.report-block p { color: var(--muted, #7d6653); font-size: 13px; line-height: 1.8; }.report-block pre, .json-view pre { max-height: 300px; overflow: auto; border-radius: 9px; padding: 12px; background: #342921; color: #f4e8d6; font-family: Consolas, monospace; font-size: 11px; line-height: 1.65; white-space: pre-wrap; }.detail-facts { display: grid; gap: 10px; margin: 5px 0 17px; }.detail-facts p { display: grid; grid-template-columns: 76px 1fr; gap: 10px; margin: 0; }.detail-facts strong { color: #3b2d24; font-size: 12px; font-weight: 700; word-break: break-word; }.detail-facts strong small { display: block; margin-top: 3px; color: var(--muted, #7d6653); font-size: 10px; font-weight: 400; }.json-view { margin-top: 20px; }.json-view > span { display: block; margin-bottom: 8px; color: var(--muted, #7d6653); font-size: 10px; }
.course-panel { grid-column: 1 / -1; }.course-stats-list { display: grid; gap: 0; margin-top: 12px; }.course-stat-row { display: flex; align-items: center; justify-content: space-between; gap: 18px; border-bottom: 1px solid rgba(80, 54, 32, .08); padding: 12px 0; }.course-stat-row:last-child { border-bottom: 0; }.course-stat-row strong, .course-stat-row small { display: block; }.course-stat-row strong { color: #3b2d24; font-size: 13px; }.course-stat-row small { margin-top: 4px; color: var(--muted, #7d6653); font-size: 10px; }.course-stat-numbers { display: grid; grid-template-columns: auto auto; align-items: baseline; column-gap: 5px; min-width: 125px; text-align: right; }.course-stat-numbers b { color: #39724e; font-family: Manrope, sans-serif; font-size: 18px; }.course-stat-numbers span { color: var(--muted, #7d6653); font-size: 10px; }.course-stat-numbers small { grid-column: 1 / -1; }
.drawer-layer { position: fixed; z-index: 1400; inset: 0; display: flex; justify-content: end; background: rgba(47, 36, 27, .24); backdrop-filter: blur(4px); }.drawer { width: min(620px, 100%); height: 100%; overflow-y: auto; border-left: 1px solid rgba(80, 54, 32, .15); background: #fbf4e9; box-shadow: -30px 0 80px -50px rgba(47, 36, 27, .75); animation: drawer-in .25s ease-out; }.user-drawer { width: min(720px, 100%); }.drawer-header { display: flex; align-items: start; justify-content: space-between; gap: 18px; padding: 28px 28px 21px; border-bottom: 1px solid rgba(80, 54, 32, .12); background: linear-gradient(135deg, rgba(255, 250, 240, .94), rgba(255, 240, 223, .7)); }.drawer-header .person-cell { align-items: center; }.drawer-header h2 { margin: 0 0 5px; color: #30241b; font-size: 25px; }.drawer-header small { color: var(--muted, #7d6653); font-size: 11px; }.drawer-close { color: var(--muted, #7d6653); font-size: 27px; cursor: pointer; }.drawer-tabs { display: flex; gap: 2px; overflow-x: auto; padding: 10px 20px 0; border-bottom: 1px solid rgba(80, 54, 32, .12); }.drawer-tabs button { flex: 0 0 auto; border-bottom: 2px solid transparent; padding: 10px 8px; color: var(--muted, #7d6653); font-size: 11px; cursor: pointer; }.drawer-tabs button.active { border-bottom-color: #b85c50; color: #8f352f; font-weight: 800; }.drawer-body { padding: 24px 28px 50px; }.drawer-loading { padding: 60px 28px; color: var(--muted, #7d6653); text-align: center; }.drawer-section { display: grid; gap: 18px; }.profile-summary { display: grid; grid-template-columns: repeat(4, 1fr); gap: 7px; }.profile-summary div { border-radius: 9px; padding: 10px; background: rgba(234, 217, 191, .42); }.profile-summary span, .profile-summary strong { display: block; }.profile-summary span { color: var(--muted, #7d6653); font-size: 10px; }.profile-summary strong { margin-top: 4px; color: #3b2d24; font-family: Manrope, sans-serif; font-size: 18px; }.detail-facts { border-top: 1px solid rgba(80, 54, 32, .1); padding-top: 16px; }.section-caption { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; }.section-caption h3 { margin: 0; color: #3b2d24; font-size: 18px; }.section-caption > span { color: var(--muted, #7d6653); font-size: 10px; }.drawer-list { display: grid; gap: 8px; }.drawer-list-item { display: flex; align-items: center; justify-content: space-between; gap: 12px; width: 100%; border: 1px solid rgba(80, 54, 32, .11); border-radius: 10px; padding: 12px; background: rgba(255, 250, 240, .68); text-align: left; cursor: pointer; }.drawer-list-item:hover { border-color: rgba(184, 92, 80, .42); }.drawer-list-item strong { color: #3b2d24; font-size: 12px; }.drawer-list-item > b { color: #9e3f35; font-size: 10px; }.drawer-list-item > .status-badge { flex: 0 0 auto; }.drawer-body .stack-form { margin-top: 0; }.drawer-body .primary-button { justify-self: stretch; }.booking-drawer, .report-drawer, .log-drawer { width: min(560px, 100%); }.report-body { padding-top: 22px; }
...
.compact-button .icon-mark,
.panel-link .icon-mark,
.alert-arrow,
.directory-list button > .icon-mark,
.editor-empty > .icon-mark {
  width: 16px;
  height: 16px;
}
...
@media (max-width: 767px) {
  ...
  .hero-actions .admin-account-button {
    width: 100%;
  }
  ...
  .drawer-body {
    padding: 18px 14px calc(88px + var(--safe-bottom, 0px));
  }
  ...
  .detail-facts p {
    grid-template-columns: minmax(64px, .38fr) minmax(0, 1fr);
  }
}

/* 按钮专项：后台高频操作用紧凑矩形，危险操作保持文字按钮并与主操作分开。 */
...
.admin-table-wrap:focus-visible {
  outline: 2px solid var(--cinnabar, #b85c50);
  outline-offset: -2px;
}
...
.admin-table td:first-child {
  ...
  box-shadow: 8px 0 14px -14px rgba(47, 36, 27, .7);
}
...
/* 后台辅助信息统一使用可读的次级文本色，避免浅色小字在纸面上失去层次。 */
.admin-shell .admin-table th { color: var(--gold-deep, #8b5a14); }
.admin-shell .admin-table td small,
.admin-shell .person-cell small,
.admin-shell .drawer-list-item small,
.admin-shell .team-row small,
.admin-shell .course-admin-row small,
.admin-shell .drawer-header small,
.admin-shell .drawer-tabs button,
.admin-shell .drawer-loading,
.admin-shell .profile-summary span,
.admin-shell .section-caption > span,
.admin-shell .report-meta-grid span,
.admin-shell .detail-facts span,
.admin-shell .json-view > span,
.admin-shell .date-filter span,
.admin-shell .panel-note,
.admin-shell .panel-heading > span,
.admin-shell .alert-item small,
.admin-shell .distribution-title small,
.admin-shell .activity-item p,
.admin-shell .activity-item time,
.admin-shell .import-panel p,
.admin-shell .directory-list button small,
.admin-shell .calendar-card-top small,
.admin-shell .entry-toolbar small,
.admin-shell .entry-editor-head label,
.admin-shell .editor-banner span:last-child,
.admin-shell .invite-result span,
.admin-shell .course-progress-editor span,
.admin-shell .course-stat-row small,
.admin-shell .course-stat-numbers span {
  color: var(--muted, #7d6653);
  font-size: 11px;
}
